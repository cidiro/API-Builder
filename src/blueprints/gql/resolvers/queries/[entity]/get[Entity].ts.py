...
def build(entity):
    return (
        ...
        f"const get{entity.name} = async (_: unknown, args: {{ id: string }}): Promise<{entity.name}ModelType> => {{\n"
        f"  const {entity.name.lower()} = await {entity.name}Model.findById(args.id).exec();\n"
        ...
        f"export default get{entity.name};\n"
    )
